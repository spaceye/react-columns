...
export class DynamicColumns extends Component {
  ...
  componentWillUpdate(newProps, newState) {
    this.postsChanged = newProps.posts.length !== this.props.posts.length;
    this.columnsChanged = newState.columns !== this.state.columns;
  }

  componentDidUpdate() {
    if (this.postsChanged || this.columnsChanged) {
      this.setItems();
    }
  }
  ...
  getDimensions() {
    ...
    return map(this.props.posts, ({ id, w, h }) => {
  ...
  setItems() {
    ...
    const itemsDimensions = !this.columnsChanged && this.postsChanged ? { ...this.state.itemsDimensions } : {};

    map(posts, (post) => {
      const { id } = post;
      const ref = this.items[id];
  ...
  renderItems() {
    ...
    return map(items, item =>
      (<Post
        ...
        isInvisible={this.getIsItemInvisible(post.id)}
        key={item.id}
        {...item}
        {...itemProps}
      />)
  ...
  render() {
    const { posts, queries } = this.props;
